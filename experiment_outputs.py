...
def _resolved_suffix_v21_config(args):
    defaults = {
        "enabled": False,
        "log_enabled": True,
        "layer_offsets": [0, 1, 2],
    ...
    prefixes = {
        "enabled": "suffix_reoptimization_v2_1",
        "log_enabled": "suffix_reoptimization_v2_1_log",
    }
    resolved = {
        key: getattr(args, prefixes.get(key, "suffix_v2_1_" + key), default)
        for key, default in defaults.items()
    }
    resolved.update({
        "version": "v2.1",
        ...
        if selected_advanced_method == "suffix_reoptimization_v2.1":
            ...
            if baseline_accuracy is None:
                baseline_accuracy = _excel_float(
                    experiment_view.get("pre_acc")
                )
            if suffix_accuracy is None:
                suffix_accuracy = _excel_float(
                    suffix_result.get("final_accuracy")
                )
            if suffix_accuracy is None:
                suffix_accuracy = _excel_float(
                    experiment_view.get("post_acc")
                )
            if suffix_accuracy is None:
                suffix_accuracy = _excel_float(record.get("accuracy"))
    elif cgmr_enabled:
