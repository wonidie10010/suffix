...
def _config(**overrides):
    ...
    return v21.SuffixReoptimizationV21Config(**values)
...
def _run_fixture(config=None, total_input_ids=None, custom_fixed=False):
    ...
    final_embedding, result = v21.run_suffix_reoptimization_v2_1(
        model=model,
...
class ConfigAndPureFunctionTests(unittest.TestCase):
    def test_frozen_defaults_match_the_canonical_design(self):
        config = v21.SuffixReoptimizationV21Config()
    ...
    def test_config_rejects_wrong_types_nonfinite_and_changed_structure(self):
        with self.assertRaises(TypeError):
            v21.SuffixReoptimizationV21Config(global_steps=1.0)
        with self.assertRaises(ValueError):
            v21.SuffixReoptimizationV21Config(global_lr=math.inf)
        with self.assertRaises(ValueError):
            v21.SuffixReoptimizationV21Config(layer_offsets=(0, 1))
        with self.assertRaises(ValueError):
            v21.SuffixReoptimizationV21Config(global_optimizer="sgd")
        with self.assertRaises(ValueError):
            v21.SuffixReoptimizationV21Config(weight_decay_enabled=True)
        with self.assertRaises(ValueError):
            v21.SuffixReoptimizationV21Config(
    ...
    def test_config_file_has_only_v21_prefixed_method_fields(self):
        ...
        self.assertTrue(all(
            key == "suffix_reoptimization_v2_1"
            or key == "suffix_reoptimization_v2_1_log"
            or key.startswith("suffix_v2_1_")
            for key in keys
        ))
...
class LegalVocabularyTests(unittest.TestCase):
    ...
    def test_legal_count_preflight_contract_is_hard(self):
        ...
        _, result = v21.run_suffix_reoptimization_v2_1(
...
class OptimizationAndCausalFlowTests(unittest.TestCase):
    ...
    def test_uncommitted_nonfinite_local_trial_falls_back_without_pollution(self):
        ...
        with mock.patch.object(
                v21,
                "_forward_hidden",
                side_effect=v21.SuffixV21FatalError(
...
class CandidateAndFailureDomainTests(unittest.TestCase):
    ...
    def test_all_nonfinite_candidates_are_a_hard_failure(self):
        with self.assertRaisesRegex(
                v21.SuffixV21FatalError, "all_candidate_scores_nonfinite"):
    ...
    def test_explicitly_invoked_but_disabled_is_a_hard_failure(self):
        ...
        self.assertTrue(result["rollback"])
        self.assertTrue(result["fatal_failure"])
        self.assertEqual("suffix_v2_1_disabled", result["rollback_reason"])
