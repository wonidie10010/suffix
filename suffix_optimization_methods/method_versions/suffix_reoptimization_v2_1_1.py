"""Canonical suffix reoptimization v2.1.1 sidecar.
...
class SuffixV211FatalError(RuntimeError):
...
@dataclass
class SuffixReoptimizationV211Config:
    enabled: bool = False
    ...
    def __post_init__(self):
        ...
        for name in boolean_fields:
            if not isinstance(getattr(self, name), bool):
                raise TypeError("suffix_v2_1_1_{} must be boolean".format(name))
        if not isinstance(self.layer_offsets, (list, tuple)) or not all(
                isinstance(value, numbers.Integral)
                and not isinstance(value, bool)
                for value in self.layer_offsets):
            raise TypeError("suffix_v2_1_1_layer_offsets must contain integers")
        if not isinstance(self.layer_weights, (list, tuple)) or not all(
                isinstance(value, numbers.Real)
                and not isinstance(value, bool)
                for value in self.layer_weights):
            raise TypeError("suffix_v2_1_1_layer_weights must contain numbers")
        for name in integer_fields:
            value = getattr(self, name)
            if not isinstance(value, numbers.Integral) or isinstance(value, bool):
                raise TypeError("suffix_v2_1_1_{} must be an integer".format(name))
            setattr(self, name, int(value))
        for name in float_fields:
            value = getattr(self, name)
            if not isinstance(value, numbers.Real) or isinstance(value, bool):
                raise TypeError("suffix_v2_1_1_{} must be numeric".format(name))
...
def _require_finite(name, value, *, positive=False, upper=None):
    ...
    if not math.isfinite(value) or value < 0.0:
        raise ValueError(
            "suffix_v2_1_1_{} must be finite and non-negative".format(name)
        )
    if positive and value <= 0.0:
        raise ValueError("suffix_v2_1_1_{} must be positive".format(name))
    if upper is not None and value > float(upper):
        raise ValueError(
            "suffix_v2_1_1_{} must be at most {}".format(name, upper)
        )


def _validate_config(config):
    if config.layer_offsets != (0, 1, 2):
        raise ValueError("suffix_v2_1_1_layer_offsets must be exactly [0, 1, 2]")
    if len(config.layer_weights) != 3:
        raise ValueError("suffix_v2_1_1_layer_weights must contain three values")
    for value in config.layer_weights:
        _require_finite("layer_weights", value)
    if sum(config.layer_weights) <= 0.0:
        raise ValueError("suffix_v2_1_1_layer_weights must have a positive sum")
    for name in (
        "alpha_dir",
        "alpha_mag",
        "vocab_weight",
    ...
    if config.alpha_dir + config.alpha_mag <= 0.0:
        raise ValueError("suffix_v2_1_1 alpha weights must have a positive sum")
    for name in (
        "vocab_temperature",
        "global_lr",
        "local_lr",
    ...
    for name in ("adam_beta1", "adam_beta2"):
        ...
        if getattr(config, name) >= 1.0:
            raise ValueError("suffix_v2_1_1_{} must be below 1".format(name))
    ...
    if config.embedding_top_k_expanded <= config.embedding_top_k_normal:
        raise ValueError(
            "suffix_v2_1_1 expanded embedding top-k must exceed normal top-k"
        )
    fixed_values = {
    ...
    for name, expected in fixed_values.items():
        if getattr(config, name) != expected:
            raise ValueError(
                "suffix_v2_1_1_{} must be {!r}".format(name, expected)
            )
    if config.weight_decay_enabled:
        raise ValueError("suffix_v2_1_1_weight_decay_enabled must be false")
    return config
...
def resolve_effective_layers(target_layer, model_layer_count, offsets, weights):
    ...
    if not effective:
        raise ValueError("suffix v2.1.1 has no effective layers")
    total = sum(weight for _, weight in effective)
    if not math.isfinite(total) or total <= 0.0:
        raise ValueError("suffix v2.1.1 effective layer weights sum to zero")
    return (
...
def _joint_error(current_hidden, target_hidden, config):
    ...
    if current.shape != target.shape:
        raise SuffixV211FatalError("hidden_shape_mismatch", "hidden_error")
...
def build_entry_snapshot_from_embedding(
    ...
    if snapshot.ndim != 3 or int(snapshot.shape[0]) != 1:
        raise SuffixV211FatalError("invalid_entry_embedding_shape", "entry_snapshot")
    ...
    if eval_start_pos < 0 or eval_start_pos > sequence_length:
        raise SuffixV211FatalError("invalid_eval_start_pos", "entry_snapshot")
    if len(prefix) != eval_start_pos:
        raise SuffixV211FatalError("fixed_prefix_length_mismatch", "entry_snapshot")
    if attention_mask is None:
        active = [1] * sequence_length
    else:
        mask = torch.as_tensor(attention_mask)
        if mask.shape != (1, sequence_length):
            raise SuffixV211FatalError("attention_mask_shape_mismatch", "entry_snapshot")
        active = [int(value) for value in mask[0].detach().cpu().tolist()]
        if any(value not in (0, 1) for value in active):
            raise SuffixV211FatalError("attention_mask_not_binary", "entry_snapshot")
...
def _validate_model_contract(model):
    ...
    if not type_supported or not architecture_supported:
        raise SuffixV211FatalError("unsupported_model_family", "preflight")
    if not hasattr(model, "forward"):
        raise SuffixV211FatalError("causal_lm_forward_unavailable", "preflight")
...
def _register_layer_hooks(model, layer_ids, destination):
    ...
    if registered != requested:
        for handle in handles:
            handle.remove()
        raise SuffixV211FatalError("missing_layer_hooks", "preflight")
    return handles
...
def _forward_hidden(
    ...
    if input_ids is not None:
        ...
        for handle in handles:
            handle.remove()
        raise SuffixV211FatalError("missing_forward_input", "forward")
    ...
    if set(collected) != {int(value) for value in layer_ids}:
        raise SuffixV211FatalError("missing_hidden_state", "forward")
    if require_finite and any(
            not torch.isfinite(value).all() for value in collected.values()):
        raise SuffixV211FatalError("nonfinite_hidden_state", "forward")
    if return_output:
...
def _multi_hidden_error(collected, targets, layers, weights, positions, config):
    if not positions:
        raise SuffixV211FatalError("empty_effective_positions", "hidden_error")
...
def _try_vector_repair(
    ...
    try:
        ...
        reason = (
            error.reason
            if isinstance(error, SuffixV211FatalError)
...
def _snapshot_tokens(entry_token_snapshot):
    if entry_token_snapshot is None or isinstance(
        ...
        raise SuffixV211FatalError("missing_entry_token_snapshot", "preflight")
    values = list(entry_token_snapshot)
    if not values:
        raise SuffixV211FatalError("empty_entry_token_snapshot", "preflight")
    if any(
        isinstance(value, bool) or not isinstance(value, numbers.Integral)
        for value in values
    ):
        raise SuffixV211FatalError("invalid_entry_token_snapshot", "preflight")
...
def _preflight(
    ...
    if not config.enabled:
        raise SuffixV211FatalError("suffix_v2_1_1_disabled", "preflight")
    _validate_model_contract(model)
    if embed_layer is None or not hasattr(embed_layer, "weight"):
        raise SuffixV211FatalError("embedding_layer_unavailable", "preflight")
    ...
    if entry_embedding.ndim != 3 or int(entry_embedding.shape[0]) != 1:
        raise SuffixV211FatalError("invalid_entry_embedding_shape", "preflight")
    if not torch.isfinite(entry_embedding).all():
        raise SuffixV211FatalError("nonfinite_entry_embedding", "preflight")
    ...
    if len(entry_tokens) != sequence_length:
        raise SuffixV211FatalError("token_sequence_length_changed", "preflight")
    if int(entry_embedding.shape[2]) != int(embed_layer.weight.shape[1]):
        raise SuffixV211FatalError("embedding_dimension_mismatch", "preflight")
    if entry_embedding.device != embed_layer.weight.device:
        raise SuffixV211FatalError("embedding_device_mismatch", "preflight")
    eval_start_pos = int(eval_start_pos)
    if eval_start_pos <= 0 or eval_start_pos >= sequence_length:
        raise SuffixV211FatalError("invalid_eval_start_pos", "preflight")
    special_ids = {
    ...
    if attention_mask is None:
        ...
        if any(
            ...
            raise SuffixV211FatalError("attention_mask_not_binary", "preflight")
    ...
    if not positions:
        raise SuffixV211FatalError("empty_effective_positions", "preflight")
    ...
    if decoder_layers is None or len(decoder_layers) != int(model_layer_count):
        raise SuffixV211FatalError("model_layer_count_mismatch", "preflight")
    if not isinstance(target_hidden_states, dict) or set(
            target_hidden_states
    ) != set(layers):
        raise SuffixV211FatalError("target_hidden_layers_mismatch", "preflight")
    for layer_id_value in layers:
        ...
        if target.ndim != 3 or int(target.shape[0]) != 1:
            raise SuffixV211FatalError("target_hidden_shape_mismatch", "preflight")
        if int(target.shape[1]) != sequence_length:
            raise SuffixV211FatalError("target_hidden_length_mismatch", "preflight")
        if not torch.isfinite(target).all():
            raise SuffixV211FatalError("nonfinite_target_hidden", "preflight")
    ...
    if len(legal_vocab.ids) < required_legal_count:
        raise SuffixV211FatalError("legal_vocab_too_small", "preflight")
...
def run_suffix_reoptimization_v2_1_1(
        model, embed_layer, entry_embedding_snapshot, entry_token_snapshot,
        target_hidden_states, attention_mask, layer_id, model_layer_count,
        tokenizer, config, total_input_ids=None, eval_start_pos=0,
        log_file=None):
    """Run v2.1.1 and return ``(token_consistent_embedding, result)``.
    ...
    try:
        entry_tokens = _snapshot_tokens(entry_token_snapshot)
    except SuffixV211FatalError:
        raise
    if not config.enabled:
        fatal = SuffixV211FatalError("suffix_v2_1_1_disabled", "preflight")
    ...
    try:
        if not hasattr(model, "eval") or not hasattr(model, "train"):
            raise SuffixV211FatalError("causal_lm_unavailable", "preflight")
        model.eval()
        state = _preflight(
            model,
            embed_layer,
        ...
        fatal = error if isinstance(error, SuffixV211FatalError) else (
            SuffixV211FatalError(
