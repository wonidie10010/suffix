...
## 2026-09-17：实施下一轮检查点改动

本节是当前实施状态；后文保留此前修订历史。方法仍为 v2.2.2，checkpoint schema 升为 3。

- 触发改为逐位置方向偏差的 log-mean-exp：float32、tau=0.05、D_win>0.05；定位 tolerance 从 0.05 改为 0.02，累计定位轨迹的定义不变。旧 trigger cosine 配置显式报错，防止旧新语义混用。
- 候选路径仍逐位置重选下游 token；排序与接受仍要求旧求和 cosine 严格改善，D_win 不作为额外接受门槛。
- JSONL 记录逐位置 cosine、偏差、范数、D_win、首窗口标识、候选路径与接受前后双分数，支持离线检查分数冲突和修复得失；固定 experiment.log 不变。
- 一键入口只提交 CP-on；dry-run 显示唯一任务及有效配置。保留历史 CP-off 配置和结果。正式执行检查 GPU 空闲，保存 Stage-1 快照、代码 revision 和工作树差异哈希，输出 CP-on 独立汇总。
- 验证：72 项 CPU 单元及 mock 集成回归测试通过，覆盖公式与无效输入、触发边界、旧接受规则、顺序重选和回滚、配置解析、仅 CP-on 调度、快照保留及 GPU 忙碌拒绝。真实模型实验未启动，准确率效果尚需完整实验验证。

## 2026-09-17：直接修订 v2.2.2 的段内顺序重选

本次按用户要求原地修改 v2.2.2，不新增方法版本。以下修订覆盖历史说明中“只替换一个 ID”的行为；文末原始实施记录保留其当时口径。

- 问题：修复 C 后，只对固定的 D、E 重算 hidden，无法让 D、E 重新选择 token 来适应 C′。
- 当前实现：每个 C 候选各自从原状态出发，按 C′ → D′ → E′ 的顺序调用现有候选生成和重排流程，直到当前 checkpoint 末尾。D 使用 C′ 的前缀，E 使用 C′、D′ 的前缀；段前位置固定。
- 接受：对每条完整重选路径重新计算原段级求和 cosine；所有候选成功评估后，只有最佳路径严格优于原段才整体提交 token、embedding 和候选表。无改善或任一候选评估失败时保留原状态；契约错误仍终止运行。
- 范围：“每段最多一次修复”现在指一次修复事务、一个定位起点，可改变起点及其后的多个 token；同段不递归诊断。checkpoint 之后的候选上下文失效，继续原有未来位置刷新流程。
- 记录：checkpoint schema 升为 2，增加顺序重选策略、各候选路径、实际变化位置及额外前向开销。离线 direct repairs / damage 统计整条已接受路径内的正确性变化。新增明细只写 JSON artifact，固定 experiment.log 不变。
- 当前参数仍为触发 cosine 0.90、定位 tolerance 0.05、候选下限 0.90。“下一轮改动”中的 D_win 和 tolerance 0.02 尚未实施。

验证：65 项本机 CPU 单元、合成及 mock 集成测试通过，覆盖真实重排函数的逐位置依赖、候选分支隔离、完整路径提交、失败回滚、离线得失统计及旧版本回归。本次未启动真实模型实验；这些检查不能证明恢复准确率提升。

## 原始实施记录（2026-09-16）

当时状态：已实施并完成本机 CPU 合成/mock 验证；真实模型验证与完整实验尚未执行。此状态不描述之后已完成的历史实验。

资料口径：以当时工作树代码、配置与测试为准；方法依据为用户确认的[实现规格与实验方案](../方法描述/v2.2.1/3检查点诊断/设计1/检查点诊断_实现规格与实验方案.md)。没有使用新实验 artifact 推断方法效果。项目引用的升级流程和更新模板在工作树中为既有删除，本次完整读取 HEAD 历史内容作为流程参考，未恢复这些文件。
