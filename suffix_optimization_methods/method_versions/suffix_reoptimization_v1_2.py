...
METHOD_NAME = "suffix_reoptimization_v1.2"
...
@dataclass
class SuffixReoptimizationV12Config:
    enabled: bool = False
...
def _find_adaptive_anomalies(embedding_scores, token_scores, scan_pos, config):
    ...
    hidden_stats = _stats(embedding_scores[start_pos:], min_points, min_std)
    ...
    for pos in range(start_pos, len(embedding_scores)):
        ...
        if token_drop_z is not None and token_drop > 0.0 and token_drop_z >= drop_z_threshold:
        ...
        if reasons:
            hidden_mean, hidden_std = hidden_stats if hidden_stats is not None else (None, None)
            token_mean, token_std = token_stats if token_stats is not None else (None, None)
            drop_mean, drop_std = drop_stats if drop_stats is not None else (None, None)
            anomalies.append({
                "position": pos,
                "reasons": reasons,
                ...
                "hidden_adaptive_cutoff": None if hidden_stats is None else hidden_mean - low_z_threshold * hidden_std,
                "hidden_adaptive_z": hidden_z,
                "token_forward_similarity": token_forward,
                "token_forward_mean": token_mean,
                "token_forward_std": token_std,
                "token_forward_cutoff": None if token_stats is None else token_mean - low_z_threshold * token_std,
                "token_forward_z": token_z,
                "token_forward_drop": token_drop,
                "token_forward_drop_mean": drop_mean,
                "token_forward_drop_std": drop_std,
                "token_forward_drop_cutoff": None if drop_stats is None else drop_mean + drop_z_threshold * drop_std,
...
def _find_anomalies(embedding_scores, token_scores, scan_pos, config):
    mode = str(getattr(config, "anomaly_detection_mode", DEFAULT_ANOMALY_DETECTION_MODE) or DEFAULT_ANOMALY_DETECTION_MODE)
    mode = mode.lower()
    if mode not in ANOMALY_DETECTION_MODES:
        raise ValueError("suffix anomaly_detection_mode must be one of: {}".format(
            ", ".join(sorted(ANOMALY_DETECTION_MODES))
        ))
    if mode == "threshold":
        return _find_threshold_anomalies(embedding_scores, token_scores, scan_pos, config)
    return _find_adaptive_anomalies(embedding_scores, token_scores, scan_pos, config)
...
def _build_suffix_hidden_weights(length, mode, decay, floor, device, dtype):
    ...
    if mode == "uniform":
        ...
        weights = torch.pow(torch.tensor(decay, device=device, dtype=dtype), positions)
        if floor > 0.0:
...
def _nearest_vocab_embeddings(suffix_param, embed_layer, invert_method="cosine",
                              chunk_size=VOCAB_SEARCH_CHUNK_SIZE):
    queries = suffix_param.detach().squeeze(0).to(dtype=torch.float32)
    if queries.dim() == 1:
    ...
    best_scores = None
    best_ids = None
    use_l2 = str(invert_method).lower() == "l2"
    normalized_queries = None if use_l2 else F.normalize(queries, dim=-1)
    with torch.no_grad():
        for start in range(0, vocab_size, chunk_size):
            end = min(start + chunk_size, vocab_size)
            weight_chunk = weight[start:end].detach().to(device=device, dtype=torch.float32)
            if use_l2:
                scores = -((queries.unsqueeze(1) - weight_chunk.unsqueeze(0)) ** 2).sum(dim=-1)
            else:
                scores = normalized_queries.matmul(F.normalize(weight_chunk, dim=-1).t())
            chunk_scores, chunk_positions = torch.max(scores, dim=1)
            chunk_ids = torch.arange(start, end, device=device, dtype=torch.long)[chunk_positions]
            ...
            best_ids = torch.where(improved, chunk_ids, best_ids)

        nearest = weight.index_select(0, best_ids.to(weight.device))
        nearest = nearest.detach().to(device=device, dtype=torch.float32).unsqueeze(0)
    return nearest
...
def _embedding_range_bound(embed_layer, device, dtype, top_k=10,
    ...
    with torch.no_grad():
        for start in range(0, vocab_size, chunk_size):
            ...
            chunk = weight[start:end].detach().abs().to(device=device, dtype=torch.float32)
...
def _optimize_suffix(model, input_embed, target_hidden_state, attention_mask, layer_id,
    ...
    if float(config.range_weight) > 0.0:
        range_bound = _embedding_range_bound(embed_layer, suffix_param.device, suffix_param.dtype)
    ...
    for epoch_idx in range(max(0, int(config.epoch))):
        ...
        weighted_hidden_loss = (per_pos_hidden_loss * hidden_weights).sum() / hidden_weights.sum().clamp_min(1e-12)
        prox_loss = F.mse_loss(suffix_param, original_suffix)
        manifold_loss = torch.zeros((), device=suffix_param.device, dtype=suffix_param.dtype)
        if float(config.manifold_weight) > 0.0 and epoch_idx >= manifold_warmup_epoch:
            if nearest_vocab_embedding is None or (epoch_idx - manifold_warmup_epoch) % manifold_update_every == 0:
                ...
                manifold_updates += 1
            manifold_loss = F.mse_loss(suffix_param, nearest_vocab_embedding.detach())
        range_loss = torch.zeros((), device=suffix_param.device, dtype=suffix_param.dtype)
    ...
    return _merge_suffix(base_embed, suffix_start, suffix_param).detach(), optimization_summary
...
def _log_result(f, result):
    ...
    for event in result.get("events", []):
        if event.get("triggered"):
            ...
            log_line(f, (
                "  suffix_round {}/{}: anomaly_pos={}, reasons={}, mode={}, score={}, weight_mode={}, "
                "hidden_loss={} -> {}, prox_loss={} -> {}, manifold_loss={} -> {}, "
                "range_loss={} -> {}, loss={} -> {}, manifold_updates={}, "
                "accept_mode={}, accepted={}, changed_positions={}, before_acc={}, candidate_acc={}, "
                "before_hidden_mean={}, candidate_hidden_mean={}, "
                "before_hidden_min={}, candidate_hidden_min={}, "
                "before_anomalies={}, candidate_anomalies={}, accept_reason={}, stopped_reason={}"
            ).format(
                event.get("round"),
                event.get("max_rounds"),
                event.get("anomaly_position"),
                event.get("anomaly_reasons"),
                event.get("anomaly_detection_mode"),
                format_metric(event.get("anomaly_score")),
                optimization.get("hidden_weight_mode"),
                ...
                optimization.get("manifold_updates"),
                ...
                optimization.get("stopped_reason"),
            ), console=False)
        else:
            log_line(f, (
...
def run_suffix_reoptimization_v1_2(model, embed_layer, optimized_embedding, target_hidden_state,
...
SuffixReoptimizationConfig = SuffixReoptimizationV12Config
run_suffix_reoptimization = run_suffix_reoptimization_v1_2
