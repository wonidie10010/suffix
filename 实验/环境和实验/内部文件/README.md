...
- `实验/一键运行_suffix_v2_1.py`：suffix v2.1 独立入口。

两个入口都只启动一个实验进程，只使用 `DEML_GPU_ID` 指定的一张 GPU；v2.1
不会选择 v2.0 或任何 CGMR 方法。v2.1 的正式配置是仓库根目录中的
`experiment_configs/l24_airport_medical_suffix_v2_1_no_cgmr.json`。

启动器直接使用仓库根目录中的项目源码、`requirements.txt` 和实验配置，不维护
内嵌项目副本。v2.0/v2.1 复用上一级目录的共享 `.runtime/`、Conda 环境和模型
