...
LOCK_FILE="${RUNTIME_DIR}/suffix-experiment.lock"
...
acquire_runtime_lock() {
    if ! command -v flock >/dev/null 2>&1; then
        fail_experiment "系统缺少 flock，无法保护共享运行环境" 3
    fi
    if ! exec 9>"${LOCK_FILE}"; then
        fail_experiment "无法创建共享运行环境锁" 3
    fi
    if ! flock -n "${LOCK_FD}"; then
        local owner=""
        owner="$(cat "${LOCK_OWNER_FILE}" 2>/dev/null || true)"
        if [[ -n "${owner}" ]]; then
            fail_experiment "共享运行环境正在被 PID ${owner} 使用" 8
        fi
        fail_experiment "共享运行环境正在被其他一键实验使用" 8
    fi
    LOCK_ACQUIRED=1
    printf '%s\n' "$$" >"${LOCK_OWNER_FILE}" || \
        fail_experiment "无法记录共享运行环境锁持有者" 3
}
